...
void Display()
{
	...
	if (res)
	{
		...
		glEnable(GL_LIGHTING);
		Pattern->SetUniformVariable((char *)"uKa", (float)0.25);
		SetPointLight(GL_LIGHT0, 0 , 50, 0, 1, 1, 1);
		...
		for (int i = 0; i < SHELLMAX; i++)
		{
			if ((Time - Shells[i].startTime) < SHELLDURATION && Shells[i].active)
			{
				...
				{
					//Dammage:
					IS3HP -= 2 * fabs(fabs(sin((Shells[i].angle - IS3HullAngle)* PI / 180)) - fabs(cos((Shells[i].angle - IS3HullAngle)* PI / 180)));
					// Bounce!
					if (fabs(fabs(sin((Shells[i].angle - IS3HullAngle)* PI / 180)) - fabs(cos((Shells[i].angle - IS3HullAngle)* PI / 180))) < BOUNCETHRESH)
					{
						...
						if (IS3XY[0] > Shells[i].x)
						{
							if (IS3XY[1] > Shells[i].y)
								if (IS3XY[0] - Shells[i].x >= IS3XY[1] - Shells[i].y)
									if(fabs(sin((Shells[i].angle - IS3HullAngle)* PI / 180)) - fabs(cos((Shells[i].angle - IS3HullAngle)* PI / 180)) >=0)
										Shells[i].angle += 90;
									else
										Shells[i].angle -= 90;
								else
				...
				{
					//Dammage:
					AbramHP -= 2 * fabs(fabs(sin((Shells[i].angle - AbramHullAngle)* PI / 180)) - fabs(cos((Shells[i].angle - AbramHullAngle)* PI / 180)));
					// Bounce!
					if (fabs(fabs(sin((Shells[i].angle - AbramHullAngle)* PI / 180)) - fabs(cos((Shells[i].angle - AbramHullAngle)* PI / 180))) < BOUNCETHRESH)
					{
						alSourcePlay(Sources[2]);
						if (AbramXY[0] > Shells[i].x)
						{
							if (AbramXY[1] > Shells[i].y)
								if (AbramXY[0] - Shells[i].x >= AbramXY[1] - Shells[i].y)
									if (fabs(sin((Shells[i].angle - AbramHullAngle)* PI / 180)) - fabs(cos((Shells[i].angle - AbramHullAngle)* PI / 180)) >= 0)
										Shells[i].angle += 90;
									else
										Shells[i].angle -= 90;
								else
